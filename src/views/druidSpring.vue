    ...
    <div>
        <Card>
            ...
            <Table size="small"  highlight-row ref="currentRowTable" height="500"
             border   :columns="col_spring" :data="data_spring"></Table>
        </Card>
    </div>
</template>
<script>
    export default {
        data () {
            return {
                col_spring: [
                    ...
                    {title: '执行次数',key: 'ExecuteCount',sortable: true,width:170},
                    {title: '执行时间',key: 'ExecuteTimeMillis',sortable: true,width:170},
        ...
        methods:{
            handleQuery (){
                let $this=this;
                this.$ajax.get('/druid/spring.json').then(function(res) {
                    if(res && res.data.ResultCode == 1){
                        $this.data_spring=res.data.Content;
                    }else{
